...
#
########################################################################## 
# NOTICE: updated fall of 2006 to work with existing pvcs, cvs 
########################################################################## 
...
#   does not.  A global search and replace converts spaces, commas,
#   periods, slashes (forward and backward), brackets, and braces to
#   underscore.  There may be other cases that ought to be added.
# * Any workfile within the VCS/../ or vcs/../ (or possibly ./ with '-pflat')
#   will be deleted (or overwritten) depending on your mode of
#   operation since it is used in the checkout of each revision
...
sub execdir
	...
	my ($pvcsarchive, $workfile, $workfile_nq, $rcsarchive);				# .??v, checked out file, and ,v files,
										# respectively
	...
	foreach $pvcsarchive (@pvcsarchives)
		...
		for ( $num = 0; $num < $num_vlog_strings; $num++ )
			...
			if( ( /^Workfile:\s*/ ) && (!$got_workfile ) )
				...
				print "RCS Archive is $rcsarchive\n" if ($options{debug});
				}
		...
		# Loop through each version label, checking for need to relabel  any of '[]{}.,/\ ' with '_'.
...
############ BUGFIX #####################################################################################################
# author  : Axel Baltjan (2001-06-13) 
# problem : revision number was not extracted correctly
# solution: buffering result from first call of split-command, then split by whitespace  
#           plus some additional debug-output
#           (format of string is >"[label]" = [revision]<)
 
                        print "proceed with substring '$_[2]'\n" if ($options{debug});
                        my $buf;
                        $buf = $_[2];
			split / /, $buf;
                        print "after splitting: >'$_[1]'<>'$_[2]'<>'$_[3]'\n" if ($options{debug});
			$label_revision[$label_index] = $_[2];
			print "Original revision is $label_revision[$label_index]\n" if ($options{debug});
############ BUGFIX #####################################################################################################
# original lines:
#                       split /\s+/, $_[2];
#		        $label_revision[$label_index] = $_[2];
...
#########################################################################################################################
			...
			# replace special characters with '_', if needed
			$_=$label;
		...
		foreach $revision (@rev_num)
			{
			# print "get -Y -p$revision $pvcsarchive >$workfile\n";
			print "get -Y -r$revision $pvcsarchive\n";
			# $vcs_output = `vcs -u -r$revision $pvcsarchive`;
			# $get_output = `get -Y -p$revision $pvcsarchive >$workfile`;
			$get_output = `get -Y -r$revision $pvcsarchive`;
			...
			if ( $first_time )
				...
				if ( ( @hits = grep { $workfile_nq =~ /$_/ } keys %bin_ext ) || $options{'force-binary'} )
					{
					$rcs_command .= " -kb";
					$workfile_nq =~ /$hits[0]/ if (@hits);
...
#DLM Actually, this does NOT work for trunks with major rev # > 1
#Fix it here!
			my $branch = "";
			my @branch;
			...
			print "Attempt to lock revision number $branch\n";
